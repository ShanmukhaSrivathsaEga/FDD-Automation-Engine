...
with tab_wiki:
    ...
    Use this reference guide to understand the specific accounting mechanics and data requirements behind the engine's automated tests.

    ### 🔴 Revenue Integrity & Cut-Off
    **Test 04: Execution Gap & Upfront Billing**
    *   **The Goal:** Ensure the target isn't booking revenue for physical products that haven't actually left the facility.
    *   **Books Used:** General Ledger, Warehouse Dispatch Register
    *   **Columns Targeted:** `ACCOUNT_NAME`, `PARTY_NAME`, `AMOUNT`
    *   **How it Works:** The engine compares the total billed value of physical goods in the GL against the total value physically shipped in the Warehouse log. If billing massively exceeds dispatch, it flags an execution gap, requiring deferred revenue amortization.

    **Test 06: Cashless Round-Tripping (Contra-Settlement)**
    *   **The Goal:** Identify synthetic trading volume created between related or friendly entities to artificially inflate top-line growth.
    *   **Books Used:** General Ledger
    *   **Columns Targeted:** `PARTY_NAME`, `ACCOUNT_NAME`, `AMOUNT`
    *   **How it Works:** The engine finds entities acting as both Customer and Vendor. It then checks how much of this overlapping trade volume was settled in actual cash. If the cash settlement ratio is abnormally low (e.g., < 15%), it assumes the volume is a cashless paper loop.

    **Test 17: Advance Revenue & Phantom Dispatch**
    *   **The Goal:** Prevent unearned deposits from being illegally recognized as current-year revenue under Ind AS 115 / IFRS 15.
    *   **Books Used:** General Ledger, Dispatch Register
    *   **Columns Targeted:** `NARRATION`, `PARTY_NAME`, `INVOICE_ID`, `OPERATIONAL_DATE`
    *   **How it Works:** Scans the GL for revenue entries containing keywords like "advance" or "mobilization." It then demands a corresponding physical gate pass from the Dispatch log. Without proof of delivery and transfer of control, the revenue is reversed.

    ---

    ### 🟡 Cost, Margin & Inventory
    **Test 05: Phantom Vendor Screen**
    *   **The Goal:** Catch off-book procurement, cash siphoning, or margin suppression via fake suppliers.
    *   **Books Used:** General Ledger, AP Aging Subledger
    *   **Columns Targeted:** `PARTY_NAME`, `AMOUNT`
    *   **How it Works:** Isolates all material COGS/Purchases in the General Ledger and cross-references the vendors against the official Accounts Payable directory. If a vendor generates massive costs but has zero footprint in the official AP log, it is flagged as a phantom.

    **Test 08: Overhead Absorption & Fake CWIP**
    *   **The Goal:** Stop management from artificially inflating EBITDA by hiding everyday operating expenses (OPEX) in the balance sheet as Capital Work-in-Progress (CWIP).
    *   **Books Used:** General Ledger
    *   **Columns Targeted:** `CALCULATED_TYPE`, `DATE`, `NARRATION`, `PARTY_NAME`
    *   **How it Works:** Scans for month-end "Internal Journal Vouchers" containing absorption keywords (e.g., "allocation", "capitalized to"). It reverses entries that illegally shift vague OPEX (like admin or repair costs) into long-term asset accounts.

    **Test 09: Inventory Obsolescence Write-Down**
    *   **The Goal:** Force the recognition of dead, unsellable stock that the target is keeping on the books at full value.
    *   **Books Used:** Trial Balance, Inventory Aging Report
    *   **Columns Targeted:** `CATEGORY`, `DAYS_AGED`, `VALUE`
    *   **How it Works:** The engine analyzes the physical aging of stock to find inventory older than the industry cliff (e.g., 365 days). It calculates a required provision (e.g., 50% write-off) and compares it against the existing provision in the Trial Balance. Any shortfall is deducted.

    ---

    ### 🟢 Net Debt & Working Capital
    **Test 14: AR Collectibility & ECL Analysis**
    *   **The Goal:** Ensure the target's Accounts Receivable aren't artificially inflated by uncollectible, long-overdue invoices.
    *   **Books Used:** Trial Balance, AR Aging Report
    *   **Columns Targeted:** `DAYS_OVERDUE`, `VALUE`
    *   **How it Works:** Applies a dynamic Expected Credit Loss (ECL) matrix against the AR Aging profile. It calculates the mathematically required bad debt provision and flags any shortfall against the company's currently reported Trial Balance allowance.

    **Test 20: Cash Window Dressing (The Boomerang)**
    *   **The Goal:** Detect synthetic year-end cash balances created by short-term loans that are immediately reversed after the audit closes.
    *   **Books Used:** General Ledger, Bank Statement, Bank Reconciliation
    *   **Columns Targeted:** `TXN_DATE`, `NARRATION`, `AMOUNT`, `REFERENCE_NO`
    *   **How it Works:** Analyzes high-value cash inflows in the final 7 days of the year. Using text-tokenization, it scans the subsequent month's bank statement for an identical outflow to the same party, proving the cash was "window dressing" and adjusting Free Cash Flow downwards.

    **Test 25: Shadow Debt & Synthetic AR Acceleration**
    *   **The Goal:** Expose hidden supply chain financiers and normalize the Net Working Capital (NWC) peg.
    *   **Books Used:** General Ledger
    *   **Columns Targeted:** `ACCOUNT_NAME`, `DEBIT`, `AMOUNT`, `DATE`
    *   **How it Works:** Calculates the historical speed at which the company pays vendors. If velocity crashes but no late penalties appear in the P&L, it implies a shadow financier is paying the bills. Conversely, it scans for violent spikes in "Customer Discounts" in the final month, proving the seller is burning margin to synthetically collect AR early.

    ---

    ### 🟣 Statutory & HR Integrity
    **Test 12: Stealth Related Party (RPT) Collision**
    *   **The Goal:** Expose disguised insiders extracting cash from the business via "arm's length" vendor names.
    *   **Books Used:** General Ledger, Vendor Master File, HRMS Master, Related Party Directory
    *   **Columns Targeted:** `PARTY_NAME`, `ADDRESS`, `TAX_ID`
    *   **How it Works:** Cross-references the Vendor Master file against the HR Employee Directory. If an undeclared supplier shares the exact same registered address, PAN, or Tax ID as an internal employee or director, it is immediately quantified as a stealth related-party leak.

    **Test 16: Payroll Integrity & Statutory Bypass**
    *   **The Goal:** Detect ghost employees, post-termination "zombie pay", and unpaid government tax liabilities.
    *   **Books Used:** Payroll Register, HRMS Master, Statutory Challans
    *   **Columns Targeted:** `EMPLOYEE_ID`, `BANK_ACCOUNT`, `EXIT_DATE`, `UAN`
    *   **How it Works:** Matches the payroll cash payouts against the official HR files to flag multiple employees sharing one bank account (Ghost pooling) or salaries paid weeks after an exit date. It also cross-checks ERP headcount against Government Provident Fund (PF) challans to detect off-book labor.
    """)
